...
- (instancetype)init
{
    ...
    if (self = [super init]) {
        ...
        NSDate *lastDay = [NSDate dateWithTimeInterval:-24*7*60*60 sinceDate:_selectDate];//前一天
        
        NSDate *nextDat = [NSDate dateWithTimeInterval:24*7*60*60 sinceDate:_selectDate];//后一天

        // 设置时间不少于目前时间
        _pickerView.datePicker.minimumDate = lastDay;
        
        _pickerView.datePicker.maximumDate = nextDat;
